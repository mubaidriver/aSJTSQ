...
const
  _STR_KEY = '9dd14d00f5dd71bd';  {山鸡调试器经16位md5计算得到}
...
begin
  try
    m_Caption := '山鸡调试器     官方内部QQ群：949362042';
    m_Log1 := '山鸡调试器     官方内部QQ群：949362042';
    ...
    if sPath <> '' then begin
      ...
      if FileExists(sFileName) then begin
        ...
        if decryptedDataLen > 0 then begin
          ...
          m_Caption := json.Values['山鸡调试器标题'].AsType<string>;
          m_Log1 := json.Values['官方内部QQ群'].AsType<string>;
          json.Free;
...
begin
  ...
  sLog.outDebug('内测版本，针对老客户，仅用于学术交流，开发研究用');
  sLog.outDebug('使用本软件造成的直接或者间接的损失，由使用者自行承担！');
  sLog.outDebug('山鸡调试器     官方内部QQ群：949362042');
  sLog.outDebug('支持Win10-Win11，请使用原版系统');
  sLog.outDebug('招代理');
  GetWindowsNTVer;
