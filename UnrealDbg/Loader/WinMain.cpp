...
int dotCount = 0;
std::wstring baseText1(L"程序正在初始化中");
std::wstring modText;
...
void DrawProgressBar(HWND hwnd, HDC hdc, PAINTSTRUCT ps)
{
	...
	// 创建绿色画刷
	HBRUSH hGreenBrush = CreateSolidBrush(RGB(255,0,255)); // 创建绿色画刷
...
{
	// 设置文本颜色和背景颜色
	SetTextColor(hdc, RGB(50,205, 50)); // 白色
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		SetTimer(hwnd, TIMER_TEXT, 88, NULL); // 每200毫秒更新一次		
		InitThread();
		break;
	}
	case WM_DESTROY:
	...
	case USER_PROGRESS_BAR:
	{
		...
		displayText = L"正在加载" + modText + L"所需符号表(" + std::to_wstring(wParam) + L"%)";
...
)
{
	...
	if (curdir.empty())
	{
		Common::ReportSeriousError("%s[%d] 获取程序目录失败!获取服务器失败 (error: %d)", __func__, __LINE__, GetLastError());
		return 0;
	}
	curdir += L"res\\mm.pak";
	...
	{
		Common::ReportSeriousError("%s[%d] 资源文件已损坏! 获取服务器失败(error: %d)", __func__, __LINE__, GetLastError());
		return 0;
	}
...
PROCESS_INFORMATION _StartProcess_(PSTARTUP_INFO pStartInfo)
{
	...
	{
		Common::ReportSeriousError("%s[%d] 启动山鸡调试器失败!获取服务器失败 (error: %d)", __func__, __LINE__, GetLastError());
	}
	else
	{
	}
